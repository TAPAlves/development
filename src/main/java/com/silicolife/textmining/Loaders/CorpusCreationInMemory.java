...
public class CorpusCreationInMemory {

	ICorpus newCorpus;

	public CorpusCreationInMemory()
	{

	}


	public ICorpusCreateReport createCorpus(ICorpusCreateConfiguration configuration) throws IOException
	{

		Properties properties = configuration.getProperties();
		properties.put(GlobalNames.textType, CorpusTextType.convertCorpusTetTypeToString(configuration.getCorpusTextType()));
		newCorpus = new CorpusImplInMemory(configuration.getCorpusName(), configuration.getCorpusNotes(), configuration.getProperties());
		...
		for(IPublication publication:documents) {
			...
			((CorpusImplInMemory) newCorpus).addDocument(publication);
			step++;
